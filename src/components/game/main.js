...
function ImportResponse(response) {
    ...
    if (response != '0' && response && r[0] == '1') {
        ...
        Waters = Math.min(1000, parseInt(r[8]));Buyables['Water'].price = parseInt(r[9]);
        Compost = Math.min(1000, parseInt(r[10]));Buyables['Compost'].price = parseInt(r[11]);
...
Reset=function() {
    if(confirm('How the world goes round...')) {
        document.cookie='TreeSimulatorSave=0; expires=Fri, 1 Jan 2024 20:47:11 UTC;';
        ResetResponse();
    }
}
...
MakeSaveString=function() {
    ...
    parseInt(Waters)+'|'+parseInt(Buyables['Water'].price)+'|'+
    parseInt(Composts)+'|'+parseInt(Buyables['Compost'].price)+'|'
...
LoadResponse=function(response) {
    ...
    if (response != '0' && response && r[0] == '1') {
        ...
        Apples = Math.min(1000, parseInt(r[6]));Buyables['Apple'].price = parseInt(r[7]);
        Waters = Math.min(1000, parseInt(r[8]));Buyables['Water'].price = parseInt(r[9]);
        Composts = Math.min(1000, parseInt(r[10]));Buyables['Compost'].price = parseInt(r[11]);
        /*Factories = Math.min(1000, parseInt(r[12]));Buyables['Factory'].price = parseInt(r[13]);
        Machines = Math.min(1000, parseInt(r[14]));Buyables['Machine'].price = parseInt(r[15]);
        AntiMatters = Math.min(1000, parseInt(r[16]));Buyables['Anti Matter'].price = parseInt(r[17]);
        LuckMakers = Math.min(1000, parseInt(r[18]));Buyables['Luck Maker'].price = parseInt(r[19]);
...
RebuildStore=function() {
    ...
    for(var i in Buyables) {
        ...
        else if(Buyables[i].name=='Water') amount=Waters;
        else if(Buyables[i].name=='Compost') amount=Composts;
        else if(Buyables[i].name=='Test-test') amount=Tests;
        ...
        */str+='<div id="buy'+Buyables[i].name+'" onclick="Buy(\''+Buyables[i].name+'\');" style="'+(Buyables[i].name=='Elder Pledge'?'display:none;':'')+(Buyables[i].name=='Time machine'?'font-size:90%;':'')+'background-image:url(sprites/'+Buyables[i].pic+'.png); background-size: contain;"><b>'+Buyables[i].name+' <moni></moni> '+/*Beautify(Buyables[i].price)+*/'</b><br>'+Buyables[i].desc+''+(amount>0?('<div class="amount">'+amount+'</div>'):'')+'</div>';
    }
...
/*RebuildAchievements=function() {
    ...
    for(var i in Achievements) {
        str+='<div title="'+Achievements[i].name+'<br><br>'+Achievements[i].desc+'" id="'+Achievements[i].name+'" style="'+(Achievements[i].name=='Elder Pledge'?'display:none;':'')+(Achievements[i].name=='Time machine'?'font-size:90%;':'')+'background-image:url(sprites/'+Achievements[i].pic+'.png);display:none;"></div>';
    }
...
new Buyable('Apple','Lovely home-grown apples','apple',100,0,function(){Apples++; TotalApples++;});
new Buyable('Water','Even plants need hydration','water',0,0,function(){});
new Buyable('Compost','Nutrition for the soil','compost',0,1,function(){});
new Buyable('Test-test','TESTING: Apples +10, Water +100, Growth +1000%','test',0,0,function(){Trees+=1000; Waters+=100; Apples+=10; TotalApples+=10;});
...
Main=function() {
    ...
    l('tps').innerHTML=Beautify(Trees)+'% growth';
    l('apples').innerHTML=Beautify(TotalApples);

    if(Apples<=100) l('treeFloor').classList.add('small');
    else if(Apples<=10000) l('treeFloor').classList.add('medium');
    else l('treeFloor').classList.add('large');

    var str='';
    if(Apples<20) str='People like the taste of<br>your apples.';
    else if(Apples<100) str='You are getting some customers<br>around the neighborhood.';
    else if(Apples<250) str='People say your apples are of<br>very good quality.';
    else if(Apples<500) str='Companies are interested in your<br>apples.';
    else if(Apples<1000) str='People from far away come to buy<br>your apples.';
    else if(Apples<4000) str='News: apple season has doubled<br>the yearly harvests';
    else if(Apples<10000) str='News: apples stocks are climbing<br>by 20% this season!';
    else if(Apples<50000) str='The world now knows your apples!';
    else str='Oh gosh.<br>Is this even possible?!';
